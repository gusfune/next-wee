...
| `wee g layout <segment>`, `wee g handler <segment> [--methods=GET,POST]`, `wee g metadata <segment> [--sitemap] [--og]` | Layout; `route.ts` with typed methods, a Zod body schema and a test; `app/sitemap.ts` and `opengraph-image.tsx` (both when no flag). |
...
| `wee g resource <Name> <attr:type...> [--api] [--skip-migration] [--skip-loading] [--skip-error] [--skip-install]` | The whole CRUD flow in one manifest: model, `app/<plural>/actions.ts` with create/update/remove, form, list/new/show/edit pages with boundaries, `lib/display.ts`, a link in `components/nav.tsx` and `e2e/<plural>.spec.ts`. Installs `@playwright/test`. Render `<Nav />` from the root layout by hand. With `--api`: model, then `app/api/<plural>/route.ts` (GET list, POST create) and `app/api/<plural>/[id]/route.ts` (GET show, PATCH update, DELETE remove) bound to the service and validator; no UI, no Playwright install. `destroy resource <Name>` takes it all back. |
